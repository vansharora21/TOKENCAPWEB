  ...
  return (
    <PageWrapper className="max-w-7xl mx-auto flex">
      ...
      <aside className="w-64 border-r border-card-border bg-card p-6 hidden md:flex flex-col gap-4 self-stretch min-h-[calc(100vh-64px)]">
        <div className="mb-6">
          <p className="text-xl font-bold text-foreground tracking-tight">Documentation</p>
          <p className="font-mono text-[10px] text-muted mt-1 uppercase tracking-wider">v1.2.0</p>
        </div>
        ...
        <div className="mt-auto pt-6 border-t border-card-border space-y-1">
          <p className="font-mono text-[9px] font-semibold text-muted uppercase tracking-widest px-2.5 pb-2">COMMUNITY</p>
          {communityItems.map((item) => (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="flex items-center gap-3 p-2.5 rounded-lg text-sm text-muted hover:bg-pre-bg hover:text-foreground transition-all"
            >
              <span className="material-symbols-outlined text-base">{item.icon}</span>
              <span>{item.label}</span>
            </a>
      ...
      <main className="flex-grow px-6 py-12 md:px-12 min-w-0">
        <header className="mb-12">
          <h1 className="text-4xl font-bold tracking-tight text-foreground mb-3 font-nerdropol">CLI Reference</h1>
        ...
        <section className="mb-16" id="install">
          ...
          <div className="glass-panel rounded-xl overflow-hidden terminal-glow">
            <div className="bg-pre-bg px-4 py-2 flex justify-between items-center border-b border-card-border">
            ...
            <div className="p-4 font-mono text-[13px] leading-relaxed flex items-center justify-between gap-4">
              <div className="flex gap-2 overflow-x-auto scrollbar-none">
                <span className="text-emerald-500 dark:text-[#4edea3] select-none">$</span>
                <span className="text-foreground whitespace-nowrap">npm install -g tokencap</span>
              </div>
          ...
          <div className="flex flex-col lg:flex-row gap-8 justify-between items-stretch">
            <div className="flex flex-col justify-between flex-grow max-w-xl">
              <div>
                ...
                <h3 className="text-2xl font-bold text-foreground mb-2">Project Brain (brain)</h3>
              ...
              <div className="space-y-3 font-sans text-xs text-muted">
                <div className="flex items-center gap-2">
                  <span className="material-symbols-outlined text-base text-emerald-500 dark:text-[#4edea3]">check_circle</span>
            ...
            <div className="glass-panel rounded-xl overflow-hidden flex-grow flex flex-col min-w-[320px] bg-pre-bg">
              <div className="bg-pre-bg px-4 py-2 border-b border-card-border flex justify-between items-center">
                <span className="font-mono text-xs text-muted">Terminal — tokencap brain</span>
                <CopyButton text="tokencap brain auth" />
              </div>
              <div className="p-4 font-mono text-[13px] flex-grow leading-relaxed">
                <div className="flex gap-2 overflow-x-auto scrollbar-none">
                  <span className="text-[#7c3aed] dark:text-[#d2bbff] select-none">&gt;</span>
                  <span className="text-foreground whitespace-nowrap">tokencap brain auth</span>
                </div>
                <div className="mt-2 text-red-500 dark:text-red-400">Risk: <span className="font-bold">CRITICAL (🔴)</span></div>
                <div className="text-emerald-600 dark:text-[#4edea3]">✔ <span className="text-foreground">Fuzzy matched cluster: Authentication</span></div>
                <div className="text-emerald-600 dark:text-[#4edea3]">✔ <span className="text-foreground">Resolved 6 files in cluster</span></div>
                <div className="text-muted">June 3 | Session Validation Added (authService.ts)</div>
                <div className="mt-3 p-3 bg-card rounded border border-card-border text-[#7c3aed] dark:text-[#d2bbff] leading-normal text-xs overflow-x-auto scrollbar-none">
          ...
          <div className="flex flex-col lg:flex-row gap-8 justify-between items-stretch">
            <div className="flex flex-col justify-between flex-grow max-w-xl">
              <div>
                ...
                <h3 className="text-2xl font-bold text-foreground mb-2">Agent intelligence (agent)</h3>
                <p className="text-sm leading-relaxed text-muted mb-6">
            ...
            <div className="glass-panel rounded-xl overflow-hidden flex-grow flex flex-col min-w-[320px] bg-pre-bg">
              <div className="bg-pre-bg px-4 py-2 border-b border-card-border flex justify-between items-center">
                <span className="font-mono text-xs text-muted">Terminal — tokencap agent</span>
                <CopyButton text="tokencap agent --execution" />
              </div>
              <div className="p-4 font-mono text-[13px] flex-grow leading-relaxed">
                <div className="flex gap-2 overflow-x-auto scrollbar-none">
                  <span className="text-[#7c3aed] dark:text-[#d2bbff] select-none">&gt;</span>
                  <span className="text-foreground whitespace-nowrap">tokencap agent --execution</span>
                </div>
                <div className="mt-2 text-emerald-600 dark:text-[#4edea3]">✔ <span className="text-foreground">Initializing Execution Contract pipeline...</span></div>
                <div className="text-emerald-600 dark:text-[#4edea3]">✔ <span className="text-foreground">Compiling current state & engineering rules...</span></div>
                <div className="text-emerald-600 dark:text-[#4edea3]">✔ <span className="text-foreground">Activating Scope Drift Detection logic...</span></div>
                <div className="text-emerald-600 dark:text-[#4edea3]">✔ <span className="text-foreground">Generating 8 contract files...</span></div>
                <div className="mt-3 p-3 bg-card rounded border border-card-border text-[#7c3aed] dark:text-[#d2bbff] leading-normal text-xs overflow-x-auto scrollbar-none">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-16">
          ...
          <section id="ask" className="flex flex-col">
            <h3 className="text-lg font-bold text-foreground mb-2">Smart context retrieval (ask)</h3>
            <p className="text-sm leading-relaxed text-muted mb-4">
            ...
            <div className="glass-panel rounded-xl overflow-hidden terminal-glow flex-grow flex flex-col">
              <div className="bg-pre-bg px-4 py-2 border-b border-card-border flex justify-between items-center">
                <span className="font-mono text-xs text-muted">Terminal — tokencap ask</span>
                <CopyButton text="tokencap ask &quot;How does authentication work?&quot;" />
              </div>
              <div className="p-4 font-mono text-[13px] flex-grow bg-pre-bg leading-relaxed">
                <div className="flex gap-2 overflow-x-auto scrollbar-none">
                  <span className="text-[#7c3aed] dark:text-[#d2bbff] select-none">&gt;</span>
                  <span className="text-foreground whitespace-nowrap">tokencap ask "How does authentication work?"</span>
                </div>
                <div className="mt-3 text-emerald-600 dark:text-[#4edea3]">✔ <span className="text-foreground">Analyzing question keywords...</span></div>
                <div className="text-emerald-600 dark:text-[#4edea3]">✔ <span className="text-foreground">Matching clusters: Authentication (95%)</span></div>
                <div className="text-emerald-600 dark:text-[#4edea3]">✔ <span className="text-foreground">Performing 2-hop BFS import search...</span></div>
                <div className="mt-3 p-3 bg-card rounded border border-card-border text-[#7c3aed] dark:text-[#d2bbff] leading-normal text-xs overflow-x-auto scrollbar-none">
          ...
          <section id="context" className="flex flex-col">
            <h3 className="text-lg font-bold text-foreground mb-2">Topic context (context)</h3>
            <p className="text-sm leading-relaxed text-muted mb-4">
              Manually compile prompt-optimized context files for a specific keyword or topic.
            </p>
            <div className="glass-panel rounded-xl overflow-hidden terminal-glow flex-grow flex flex-col">
              <div className="bg-pre-bg px-4 py-2 border-b border-card-border flex justify-between items-center">
                <span className="font-mono text-xs text-muted">Terminal — tokencap context</span>
                <CopyButton text="tokencap context payments --budget 8000" />
              </div>
              <div className="p-4 font-mono text-[13px] flex-grow bg-pre-bg leading-relaxed">
                <div className="flex gap-2 overflow-x-auto scrollbar-none">
                  <span className="text-[#7c3aed] dark:text-[#d2bbff] select-none">&gt;</span>
                  <span className="text-foreground whitespace-nowrap">tokencap context payments --budget 8000</span>
                </div>
                <div className="mt-3 text-emerald-600 dark:text-[#4edea3]">✔ <span className="text-foreground">Compiling payments context...</span></div>
                <div className="text-muted">[14:04:12] Token Budget Allocated: 8,000 tokens</div>
                <div className="mt-3 p-3 bg-card rounded border border-card-border text-[#7c3aed] dark:text-[#d2bbff] leading-normal text-xs overflow-x-auto scrollbar-none">
          ...
          <section id="snapshot" className="flex flex-col">
            <h3 className="text-lg font-bold text-foreground mb-2">Generate snapshot (make)</h3>
            <p className="text-sm leading-relaxed text-muted mb-4">
              Generate all three files: main snapshot, project knowledge graph, and developer context memory in one command.
            </p>
            <div className="glass-panel rounded-xl overflow-hidden terminal-glow flex-grow flex flex-col">
              <div className="bg-pre-bg px-4 py-2 border-b border-card-border flex justify-between items-center">
                <span className="font-mono text-xs text-muted">Terminal — tokencap make</span>
                <CopyButton text="tokencap make --profile balanced" />
              </div>
              <div className="p-4 font-mono text-[13px] flex-grow bg-pre-bg leading-relaxed">
                <div className="flex gap-2 overflow-x-auto scrollbar-none">
                  <span className="text-[#7c3aed] dark:text-[#d2bbff] select-none">&gt;</span>
                  <span className="text-foreground whitespace-nowrap">tokencap make --profile balanced</span>
                </div>
                <div className="mt-3 text-emerald-600 dark:text-[#4edea3]">✔ <span className="text-foreground">Scanning files...</span></div>
                <div className="text-emerald-600 dark:text-[#4edea3]">✔ <span className="text-foreground">Generating TOKENCAP_GRAPH.md...</span></div>
                <div className="text-emerald-600 dark:text-[#4edea3]">✔ <span className="text-foreground">Generating TOKENCAP_MEMORY.md...</span></div>
                <div className="mt-3 p-3 bg-card rounded border border-card-border text-[#7c3aed] dark:text-[#d2bbff] leading-normal text-xs overflow-x-auto scrollbar-none">
          ...
          <section id="watch" className="flex flex-col">
            <h3 className="text-lg font-bold text-foreground mb-2">Watch mode</h3>
            <p className="text-sm leading-relaxed text-muted mb-4">
              Start a background watcher that regenerates all snapshot files automatically whenever a file changes.
            </p>
            <div className="glass-panel rounded-xl overflow-hidden terminal-glow flex-grow flex flex-col">
              <div className="bg-pre-bg px-4 py-2 border-b border-card-border flex justify-between items-center">
                <span className="font-mono text-xs text-muted">Terminal — tokencap watch</span>
                <CopyButton text="tokencap watch --debounce 3000" />
              </div>
              <div className="p-4 font-mono text-[13px] flex-grow bg-pre-bg leading-relaxed">
                <div className="flex gap-2 overflow-x-auto scrollbar-none">
                  <span className="text-[#7c3aed] dark:text-[#d2bbff] select-none">&gt;</span>
                  <span className="text-foreground whitespace-nowrap">tokencap watch --debounce 3000</span>
                </div>
                <div className="mt-3 text-muted whitespace-nowrap overflow-x-auto scrollbar-none">[14:02:21] Watching workspace for changes...</div>
                <div className="text-emerald-600 dark:text-emerald-500 whitespace-nowrap overflow-x-auto scrollbar-none">● <span className="text-foreground">Change detected in src/app/login/page.tsx</span></div>
                <div className="text-muted whitespace-nowrap overflow-x-auto scrollbar-none">[14:02:45] Debouncing and regenerating snapshot files...</div>
                <div className="cli-cursor text-foreground mt-1"></div>
              </div>
            </div>
          </section>
          ...
          <section id="pack" className="flex flex-col">
            <h3 className="text-lg font-bold text-foreground mb-2">Context packing (pack)</h3>
            <p className="text-sm leading-relaxed text-muted mb-4">
              Pack your project workspace into a token-budgeted, importance-scored context pack for LLMs.
            </p>
            <div className="glass-panel rounded-xl overflow-hidden terminal-glow flex-grow flex flex-col">
              <div className="bg-pre-bg px-4 py-2 border-b border-card-border flex justify-between items-center">
                <span className="font-mono text-xs text-muted">Terminal — tokencap pack</span>
                <CopyButton text="tokencap pack --mode review --budget 20000" />
              </div>
              <div className="p-4 font-mono text-[13px] flex-grow bg-pre-bg leading-relaxed">
                <div className="flex gap-2 overflow-x-auto scrollbar-none">
                  <span className="text-foreground whitespace-nowrap">tokencap pack --mode review --budget 20000</span>
                </div>
                <div className="mt-3 text-emerald-600 dark:text-[#4edea3]">✔ <span className="text-foreground">Analyzing file graph centrality...</span></div>
                <div className="text-emerald-600 dark:text-[#4edea3]">✔ <span className="text-foreground">Applying representation tiers...</span></div>
                <div className="mt-3 p-3 bg-card rounded border border-card-border text-[#7c3aed] dark:text-[#d2bbff] leading-normal text-xs overflow-x-auto scrollbar-none">
          ...
          <section id="diff" className="flex flex-col">
            ...
            <div className="glass-panel rounded-xl overflow-hidden terminal-glow flex-grow flex flex-col">
              ...
              <div className="p-4 font-mono text-[13px] flex-grow bg-pre-bg leading-relaxed">
                <div className="flex gap-2 overflow-x-auto scrollbar-none">
                  <span className="text-[#7c3aed] dark:text-[#d2bbff] select-none">&gt;</span>
                  <span className="text-foreground whitespace-nowrap">tokencap diff --staged</span>
                </div>
                <div className="mt-3 text-muted whitespace-nowrap">[14:03:01] Analyzing 5 staged files...</div>
                <div className="text-amber-500 font-bold whitespace-nowrap">Risk Rating: MEDIUM</div>
                <div className="text-foreground text-xs mt-2 p-3 bg-card rounded border border-card-border leading-normal overflow-x-auto scrollbar-none">
          ...
          <section id="constitution" className="flex flex-col lg:col-span-2">
            <h3 className="text-lg font-bold text-foreground mb-2">Repository constitution (constitution)</h3>
            <p className="text-sm leading-relaxed text-muted mb-4">
              Inspect permanent codebase laws, view compliance ratings, verify schema rules, or search for rules by severity or category.
            </p>
            <div className="glass-panel rounded-xl overflow-hidden terminal-glow flex-grow flex flex-col">
              <div className="bg-pre-bg px-4 py-2 border-b border-card-border flex justify-between items-center">
                <span className="font-mono text-xs text-muted">Terminal — tokencap constitution</span>
                <CopyButton text="tokencap constitution" />
              </div>
              <div className="p-4 font-mono text-[13px] flex-grow bg-pre-bg leading-relaxed">
                <div className="flex gap-2 overflow-x-auto scrollbar-none">
                  <span className="text-[#7c3aed] dark:text-[#d2bbff] select-none">&gt;</span>
                  <span className="text-foreground whitespace-nowrap">tokencap constitution</span>
                </div>
                <div className="mt-3 text-emerald-600 dark:text-[#4edea3]">✔ <span className="text-foreground">Loading ConstitutionGraph...</span></div>
                <div className="text-muted">Constitution Score: 92 / 100</div>
                <div className="text-red-500 font-bold mt-2">Top Rules (Never Violate):</div>
                <div className="text-muted ml-4">- 🔒 CONST-SEC-001 — Password hashing algorithm must not be changed.</div>
                <div className="text-muted ml-4">- 🔴 CONST-COMP-001 — PCI-DSS compliance required for payment routes.</div>
              </div>
            </div>
          </section>
        </div>
        ...
        <section className="mb-16" id="config">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className="space-y-4">
              ...
              <div className="p-4 border border-card-border rounded-xl bg-card flex justify-between items-start">
                <div>
                  <p className="font-mono text-[10px] text-muted mb-1.5 uppercase tracking-wider">INITIALIZE CONFIG</p>
                  <code className="font-mono text-sm text-[#7c3aed] dark:text-[#d2bbff] font-bold">tokencap init</code>
                  <p className="text-xs text-muted mt-2">Creates a default config file .tokencap.json in your workspace root.</p>
                </div>
                <CopyButton text="tokencap init" className="mt-1" />
              </div>
              <div className="p-4 border border-card-border rounded-xl bg-card flex justify-between items-start">
                <div>
                  <p className="font-mono text-[10px] text-muted mb-1.5 uppercase tracking-wider">RESOLVE CONFIG</p>
                  <code className="font-mono text-sm text-[#7c3aed] dark:text-[#d2bbff] font-bold">tokencap config</code>
                  <p className="text-xs text-muted mt-2">Prints resolved configuration tree combined with CLI overrides.</p>
                </div>
            ...
            <div className="glass-panel rounded-xl overflow-hidden terminal-glow">
              <div className="p-4 font-mono text-[13px] leading-relaxed bg-pre-bg relative">
                ...
                <div className="text-foreground mt-3 leading-normal overflow-x-auto scrollbar-none">
                  {"{"}
                  <br />
                  &nbsp;&nbsp;<span className="text-emerald-600 dark:text-[#4edea3]">"profile"</span>: <span className="text-[#7c3aed] dark:text-[#adc6ff]">"balanced"</span>,
                  <br />
                  &nbsp;&nbsp;<span className="text-emerald-600 dark:text-[#4edea3]">"outputPath"</span>: <span className="text-[#7c3aed] dark:text-[#adc6ff]">"TOKENCAP.md"</span>,
                  <br />
                  &nbsp;&nbsp;<span className="text-emerald-600 dark:text-[#4edea3]">"maxFiles"</span>: <span className="text-[#7c3aed] dark:text-[#d2bbff]">90</span>,
                  <br />
                  &nbsp;&nbsp;<span className="text-emerald-600 dark:text-[#4edea3]">"maxSourceBytes"</span>: <span className="text-[#7c3aed] dark:text-[#d2bbff]">220000</span>,
                  <br />
                  &nbsp;&nbsp;<span className="text-emerald-600 dark:text-[#4edea3]">"includeGitDiff"</span>: <span className="text-[#7c3aed] dark:text-[#d2bbff]">true</span>,
                  <br />
                  &nbsp;&nbsp;<span className="text-emerald-600 dark:text-[#4edea3]">"redactSecrets"</span>: <span className="text-[#7c3aed] dark:text-[#d2bbff]">true</span>
                  <br />
                  {"}"}
                </div>
              </div>
          ...
          <div className="overflow-x-auto border border-card-border rounded-xl bg-card/30">
            <table className="w-full text-left border-collapse">
              <thead className="bg-pre-bg">
                <tr>
                  <th className="p-4 font-mono text-[11px] uppercase tracking-wider text-muted border-b border-card-border">Flag</th>
                  <th className="p-4 font-mono text-[11px] uppercase tracking-wider text-muted border-b border-card-border">Description</th>
                  <th className="p-4 font-mono text-[11px] uppercase tracking-wider text-muted border-b border-card-border">Default</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-card-border font-sans text-sm text-muted">
                {tableOptions.map((opt) => (
                  <tr key={opt.flag} className="hover:bg-pre-bg/40 transition-colors">
                    <td className="p-4 font-mono text-xs text-[#7c3aed] dark:text-[#d2bbff]">{opt.flag}</td>
                    <td className="p-4">{opt.description}</td>
                    <td className="p-4 text-xs font-mono text-muted">{opt.defaultVal}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <section className="relative h-64 rounded-xl overflow-hidden">
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-background to-transparent flex items-end p-8">
            <div>
              <h4 className="text-xl font-bold text-foreground mb-1">Built for speed.</h4>
              <p className="text-sm text-muted">Highly optimized JS parser engine ensures execution times in milliseconds.</p>
            </div>
          </div>
        </section>
      </main>
